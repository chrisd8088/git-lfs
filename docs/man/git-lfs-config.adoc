...
setting either variable to `true`, `1`, `on`, `yes`, or `t`.
* `GIT_LFS_SKIP_SMUDGE`
+
...
behaviour and have all files writeable by setting either variable to
`false`, `0`, `off`, `no`, or `f`.
...
`false`; you can enable this behavior by setting the variable to
`true`, `1`, `on`, `yes`, or `t`.
* `lfs.defaulttokenttl`
+
