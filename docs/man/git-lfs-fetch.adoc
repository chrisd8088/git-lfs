...
`-j`::
`--json`::
  Writes the details of all object transfer requests as JSON to STDOUT,
  if the command exits successfully. When `--dry-run` is also specified,
  writes the details of the transfers that would occur if the objects
  were fetched. Intended for interoperation with external tools.
  If the command returns with a non-zero exit code, plain text messages
