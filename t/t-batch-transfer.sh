...
begin_test "batch transfers with ssh absolute path bare endpoint (git-lfs-transfer)"
(
  set -e

  setup_pure_ssh

  reponame="batch-ssh-transfer-bare-abspath"
  setup_remote_repo "$reponame"
  clone_repo "$reponame" "$reponame"
...
begin_test "batch transfers with ssh relative path bare endpoint (git-lfs-transfer)"
(
  set -e

  setup_pure_ssh

  reponame="batch-ssh-transfer-bare-relpath"
  setup_remote_repo "$reponame"
  clone_repo "$reponame" "$reponame"
...
begin_test "batch transfers with ssh custom port endpoint (git-lfs-transfer)"
(
  set -e

  setup_pure_ssh

  reponame="batch-ssh-transfer-port"
  setup_remote_repo "$reponame"
  clone_repo "$reponame" "$reponame"

  sshurl=$(ssh_remote "$reponame" 22)
  git config lfs.url "$sshurl"
